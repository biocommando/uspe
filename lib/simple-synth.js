...
class SimpleSynth {
    ...
    noteOn (note, init, lengthMs) {
        console.log('Trigger note', note, init, lengthMs)
    ...
    updateSequence(sequence) {
        if (this.sequence) {
            ...
            console.log('Updated sequence', JSON.stringify(this.sequence))
        }
    }
...
const b64ToNum = base64_string => {
    const ints = Buffer.from(base64_string, 'base64')
    const arr = []
    for (let i = 0; i < ints.length; i++) {
        arr.push(ints[i] / 128 - 1)
    }
    return arr
}

const {translateNote} = require('./translate-note')

// Sequence format:
// <note_num>-<note_length>,<note_num>-<note_length>,...@<position>;...
const createSynth = (synthJsonFile, sequence, tempo, _sampleRate, setEndOfLastNote) => {
    sampleRate = _sampleRate
    const steps = sequence.split(';')
    const noteData = []
    let endOfLastNoteVal = 0
    steps.forEach(step => {
        const notesAndPosition = step.split('@')
        const position = Number(notesAndPosition[1]) / tempo * 15 * 1000
        const notesAndLengths = notesAndPosition[0].split(',')
        noteData.push({
            position,
            notes: notesAndLengths.map(n => {
                n = n.split('-')
                endOfLastNoteVal = Math.max(endOfLastNoteVal, Number(notesAndPosition[1]) + Number(n[1]))
                return { note: translateNote(n[0]), preset: 'preset1', delaySend: 0, lengthMs: Number(n[1]) / tempo * 15 * 1000}
            })
        })
    })
    setEndOfLastNote(endOfLastNoteVal + 16) // 1 bar extra for release
    const presetData = JSON.parse(require('fs').readFileSync(synthJsonFile).toString())
    presetData.oscShape = b64ToNum(presetData.oscShape)
    
    const synth = new SimpleSynth()
    synth.handlePortEvent({ type: 'update-sequence', sequence: noteData })
    synth.handlePortEvent({ type: 'preset', presetId: 'preset1', presetData })
    synth.handlePortEvent({ type: 'start-sequence', sequence: noteData, offset: 0 })
    const buf = [0]
    return (channel) => {
        if (channel === 0) {
            synth.process(undefined, [[buf]])
        }
        return buf[0]
    }
}
